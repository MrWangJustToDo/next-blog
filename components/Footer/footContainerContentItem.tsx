...
import { FootContainerContentItemType } from "./@type";
...
FootContainerContentItem = ({ column = 1, head = "hello", content = "hello", icon = "ri-links-line", hrefTo = "/", title = content }) => {
  return (
    <li className="text-truncate my-1">
      {column === 1 ? (
        <IconItem icon={icon} content={content} hrefTo={hrefTo} title={title} />
      ) : (
        <ColumnItem head={head} content={content} title={title} />
      )}
    </li>
  );
};
