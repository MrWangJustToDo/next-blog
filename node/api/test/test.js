...
const test_registerUser = autoActionHandler({
  actionHandler: async ({ res }) => {
    await insertUser({
      db: global.db,
      ip: "127.0.0.1",
...
const test_publishHome = autoActionHandler({
  actionHandler: async ({ res }) => {
    await insertHome({
      db: global.db,
      authorId: "1",
...
const test_insertUserEx = autoActionHandler({
  actionHandler: async ({ res }) => {
...
const test_insertType = autoActionHandler({
  actionHandler: async ({ res }) => {
    await insertType({ db: global.db, typeId: 1, typeState: 1, typeContent: "java", typeCount: 1 });
    success(res, 200);
  },
});

const test_insertTag = autoActionHandler({
  actionHandler: async ({ res }) => {
    await insertTag({ db: global.db, tagId: 1, tagState: 1, tagContent: "前端", tagCount: 1 });
    success(res, 200);
  },
});

const test_primaryComment = autoActionHandler({
  actionHandler: async ({ res }) => {
...
const test_childComment = autoActionHandler({
  actionHandler: async ({ res }) => {
